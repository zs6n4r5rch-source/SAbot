...
class ExportService:
    def build_xlsx(self, name: str, headers: list[str], rows: Iterable[Mapping[str, Any]], *, filters: Mapping[str, Any] | None = None, totals: Mapping[str, Any] | None = None, timezone_name: str | None = None) -> Path:
        ...
        if filters or timezone_name:
            meta = wb.create_sheet("Параметры")
            meta.append(["Параметр", "Значение"])
            if timezone_name: meta.append(["Timezone", timezone_name])
            for k, v in (filters or {}).items(): meta.append([k, v])
            for k, v in (totals or {}).items(): meta.append([f"Итого: {k}", v])
