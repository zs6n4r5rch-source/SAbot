...
class TelegramUser(Base):
    ...
    __table_args__ = (CheckConstraint("role IN ('admin', 'owner')", name="ck_telegram_users_role"),)


class AccessProfile(Base):
    """Preconfigured real staff profile matched by Telegram username."""
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, onupdate=utcnow)
    __table_args__ = (CheckConstraint("role IN ('admin', 'owner')", name="ck_access_profiles_role"),)


class TelegramBindingRequest(Base):
    """A Telegram user claims a preconfigured staff profile; owner approves/rejects."""
...
# Client / loyalty layer. LANGAME remains the source of truth for guest and group data.
class Guest(Base):
