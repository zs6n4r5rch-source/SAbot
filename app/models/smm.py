...
from sqlalchemy import BigInteger, Boolean, DateTime, ForeignKey, Integer, Numeric, String, Text, CheckConstraint, UniqueConstraint
...
class SMMAccess(Base):
    """Dedicated SMM role/access layer; keeps existing admin/owner role constraints intact."""
    ...
    analytics_access: Mapped[str] = mapped_column(String(1000), default="social,guests,marketing,advertising", server_default="social,guests,marketing,advertising")
...
class SMMTask(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('submitted','approved','rejected')", name="ck_smm_tasks_status"),
    )
