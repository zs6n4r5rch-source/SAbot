...
async def my_shift(request: Request):
    ...
    async with SessionLocal() as session:
        ...
        return {
            "state": state, "id": shift.id, "langame_shift_id": shift.langame_shift_id,
            "started_at": iso(shift.started_at), "ended_at": iso(shift.ended_at),
            "cash_sales": dec(shift.cash_sales), "card_sales": dec(shift.card_sales),
            "mobile_sales": dec(shift.mobile_sales), "refunds_cash": dec(shift.refunds_cash),
            "refunds_card": dec(shift.refunds_card), "collection": dec(shift.collection),
            "actual_cash": dec(shift.actual_cash), "cash_difference": dec(shift.cash_difference),
            "report_id": report.id if report else None, "report_status": report.status if report else None,
        }
...
async def my_shift_close(request: Request):
    ...
    async with SessionLocal() as session:
        ...
        return {
            "shift": {"id": shift.id, "langame_shift_id": shift.langame_shift_id, "started_at": iso(shift.started_at), "ended_at": iso(shift.ended_at)},
            "report": {"id": report.id, "status": report.status, "cash_expected": dec(report.cash_expected), "cash_actual": dec(report.cash_actual), "cash_difference": dec(report.cash_difference), "stock_discrepancies_count": report.stock_discrepancies_count or 0, "cleaning_required": cleaning_required},
...
async def submit_my_shift_close(request: Request, payload: ShiftClosePayload):
    ...
    async with SessionLocal() as session:
        ...
        for item in expected:
            ...
            row = await session.scalar(select(ShiftCloseStockItem).where(
                ShiftCloseStockItem.report_id == report.id,
                ShiftCloseStockItem.product_id == int(item["product_id"]),
            ))
            if row is None:
        ...
        report.stock_discrepancies_count = sum(
            1 for item in expected
            if _qty(supplied[int(item["product_id"])].actual_quantity) != _qty(item["expected"])
        )
        ...
        await session.commit()
        return {
            "ok": True, "state": "submitted", "report_id": report.id, "shift_id": shift.id,
            "cash_expected": dec(report.cash_expected), "cash_actual": dec(report.cash_actual),
            "cash_difference": dec(report.cash_difference),
            "stock_discrepancies_count": report.stock_discrepancies_count or 0,
            "cleaning_performed_by": report.cleaning_performed_by,
        }
