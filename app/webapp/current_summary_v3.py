...
def _operation_amount(row):
    for key in ("sum", "amount", "balance_amount", "balance"):
        value = row.get(key)
...
async def _live_langame(now):
    ...
    return {
        ...
        "source": "LANGAME: Списание → Сессия + продажи бара",
    }
...
async def _summary(request: Request):
    ...
    try:
        ...
        live = {"active_guests": None, "active_sessions": None, "today_guests": None, "gaming_revenue": None, "source": f"LANGAME недоступен: {exc}"}
    try:
    ...
    groups = []
    try:
        raw_groups = await _guest_groups()
