    ...
    try:
        for g in _rows(await langame_client.guest_groups()):
            if g.get("id") is not None and g.get("name"):
                try: count=int((await langame_client.guests_search(groups=[int(g["id"])],size=1,page=1).get("pagination",{})).get("total",0))
