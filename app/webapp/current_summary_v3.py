...
async def _guest_api(request: Request, guest_id: int):
    ...
    owner_required(user)
    try:
        payload = await langame_client.guest_by_id(int(guest_id))
        rows = _rows(payload)
        guest = rows[0] if rows else {}
        return {"guest_id": int(guest_id), "fio": guest.get("fio") or guest.get("name") or "Без имени", "phone": guest.get("phone") or "", "balance": guest.get("balance"), "bonus_balance": guest.get("bonus_balance"), "black_list": guest.get("black_list")}
