...
JS = r'''<script>
const saAdminEsc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
window.adminDetail=async function(id){clear();setBottom(false);back();try{const d=await api(`/api/admins/${Number(id)}/shifts?days=30`);const items=d.items||[];const fmt=v=>v?new Date(v).toLocaleString('ru-RU',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}):'—';const report=s=>s.report_status==='submitted'?'<span class="badge good">Отчёт сдан</span>':s.report_status==='missing'?'<span class="badge warn">Отчёта нет</span>':'<span class="badge warn">Отчёт не завершён</span>';const html=items.length?items.map(s=>`<section class="card"><div class="admin-head"><div><div class="admin-name">${fmt(s.started_at)} · смена #${saAdminEsc(s.langame_shift_id)}</div><div class="admin-meta">${s.status==='open'?'🟢 Открыта':'⚪ Закрыта'}${s.ended_at?' · '+fmt(s.ended_at):''}</div></div>${report(s)}</div><div class="admin-main"><div class="mini"><div class="mini-label">Продажи</div><div class="mini-value">${money(s.sales)}</div></div><div class="mini"><div class="mini-label">Наличные</div><div class="mini-value">${money(s.cash_sales)}</div></div><div class="mini"><div class="mini-label">Карта</div><div class="mini-value">${money(s.card_sales)}</div></div><div class="mini"><div class="mini-label">Онлайн</div><div class="mini-value">${money(s.mobile_sales)}</div></div></div><div class="row"><div class="row-main"><div class="row-title">Расхождение кассы</div></div><div class="row-value">${s.cash_difference===null?'—':money(s.cash_difference)}</div></div><div class="row"><div class="row-main"><div class="row-title">Товары в отчёте</div><div class="row-sub">Расхождения по остаткам</div></div><div class="row-value">${s.stock_items_count??'—'} / ${s.stock_discrepancies_count??0}</div></div>${s.report_id?`<button class="primary" onclick="shiftReportOpen(${Number(s.report_id)})">📋 Открыть отчёт</button>`:''}</section>`).join(''):'<div class="empty">За последние 30 дней смен не найдено</div>';root.insertAdjacentHTML('beforeend',`<div class="section-title"><h2>${saAdminEsc(d.employee.name)}</h2><span>30 дней</span></div>${html}`)}catch(e){fail(e)}};
window.admins=async function(){clear();setBottom(false);back();try{const d=await api('/api/admins');const items=d.items||[];const active=items.filter(x=>x.shift?.status==='open').length;const submitted=d.today?.reports_submitted||0;const pending=d.today?.reports_pending||0;const html=items.length?items.map(x=>{const s=x.shift;let state='⚪ Сегодня смены нет',badge=`<span class="badge ${x.access_active?'good':'warn'}">${x.access_active?'Доступ':'Нет доступа'}</span>`;if(s?.status==='open'){state='🟢 Сейчас на смене';badge='<span class="badge good">На смене</span>'}else if(s?.status==='closed'&&s.report_status==='submitted'){state='⚪ Смена закрыта';badge='<span class="badge good">Отчёт сдан</span>'}else if(s?.status==='closed'){state='🟠 Отчёт не сдан';badge='<span class="badge warn">Отчёт</span>'}return `<section class="card admin-card" onclick="adminDetail(${Number(x.id)})"><div class="admin-head"><div><div class="admin-name">${saAdminEsc(x.name)}</div><div class="admin-meta">${state} · ${x.telegram_id?'Telegram привязан':'Telegram не привязан'}</div></div>${badge}</div>${s?`<div class="admin-main"><div class="mini"><div class="mini-label">Смена</div><div class="mini-value">${s.status==='open'?'Открыта':'Закрыта'}</div></div><div class="mini"><div class="mini-label">Продажи</div><div class="mini-value">${money(s.sales)}</div></div><div class="mini"><div class="mini-label">Отчёт</div><div class="mini-value">${s.report_status==='submitted'?'Сдан':'Не сдан'}</div></div><div class="mini"><div class="mini-label">Касса</div><div class="mini-value">${s.cash_difference===null?'—':money(s.cash_difference)}</div></div></div>`:`<div class="admin-main"><div class="mini"><div class="mini-label">Telegram</div><div class="mini-value">${x.telegram_id?'Есть':'Нет'}</div></div><div class="mini"><div class="mini-label">Langame</div><div class="mini-value">${x.langame_user_id||'—'}</div></div>`}</section>`}).join(''):'<div class="empty">Администраторов пока нет</div>';root.insertAdjacentHTML('beforeend',`<div class="section-title"><h2>Администраторы</h2><span>${items.length}</span></div><div class="kpis"><div class="kpi"><div class="kpi-label">Сейчас на смене</div><div class="kpi-value good">${active}</div></div><div class="kpi"><div class="kpi-label">Отчётов сдано сегодня</div><div class="kpi-value">${submitted}</div></div><div class="kpi"><div class="kpi-label">Отчётов требуют внимания</div><div class="kpi-value ${pending?'warn':''}">${pending}</div></div><div class="kpi"><div class="kpi-label">Всего администраторов</div><div class="kpi-value">${items.length}</div></div></div>${html}`)}catch(e){fail(e)}};
</script>'''
...
async def _index():
    ...
    # This script is injected before the static app script, which defines admins()
    # again. The final assignment runs after the static script has finished.
    bootstrap = JS.replace('</script>', "setTimeout(()=>{if(window.me?.role==='owner'||window.me?.role==='owner'){window.admins=window.admins;window.adminDetail=window.adminDetail}},0);</script>")
    return HTMLResponse(html.replace(marker, bootstrap + marker, 1))
...
def install(web_app):
    ...
    # Ensure nested shift route is evaluated before the generic admin route.
    routes = list(web_app.routes)
    web_app.routes[:] = [r for r in routes if getattr(r, 'path', None) not in {"/api/admins", "/api/admins/{employee_id}/shifts"}]
    for route in routes:
        if getattr(route, 'path', None) in {"/api/admins/{employee_id}/shifts", "/api/admins"}:
            continue
    web_app.add_api_route("/api/admins/{employee_id}/shifts", _admin_shifts, methods=["GET"], include_in_schema=False)
    web_app.add_api_route("/api/admins", _admins, methods=["GET"], include_in_schema=False)
    web_app.routes.sort(key=lambda route: 0 if getattr(route, 'path', None) == "/api/admins/{employee_id}/shifts" else 1)
