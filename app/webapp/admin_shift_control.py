...
async def _admins(request: Request):
    ...
    items = []
    for employee, tg in employees:
        items.append({
            "id": employee.id,
            "name": employee.full_name or f"Сотрудник #{employee.id}",
            "phone": employee.phone,
            "telegram_id": tg.telegram_id if tg else None,
            "role": tg.role if tg else "not_linked",
            "access_active": bool(tg and tg.active),
            "langame_user_id": employee.langame_user_id,
            "shift": latest.get(employee.id),
        })
    return {
        "items": items,
        "today": {"date": day_start.date().isoformat(), "reports_submitted": submitted, "reports_pending": pending},
    }
...
def install(web_app):
    ...
    # New routes must precede legacy handlers with the same paths.
