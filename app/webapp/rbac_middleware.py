...
class UnifiedRBACMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if path=="/api/app/overview" and role in {"admin","smm"}:
            response=JSONResponse(await safe_overview(role),status_code=200); await response(scope,receive,send); return
        if role in {"owner","admin"} and path in {"/api/app/warehouse","/api/app/warehouse/critical","/api/app/warehouse/arrivals","/api/app/warehouse/sales"}:
            try:
                ...
                if response_data is not None:
                    response=JSONResponse(response_data,status_code=200); await response(scope,receive,send); return
            except (LangameAPIError,ValueError) as exc:
                response=JSONResponse({"detail":f"LANGAME warehouse unavailable: {exc}"},status_code=502); await response(scope,receive,send); return
