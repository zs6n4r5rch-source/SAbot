...
async def _work(request: Request):
    ...
    return {
        ...
        "bar": {"sales": bar.get("sales"), "purchases": bar.get("purchases"), "profit": bar.get("profit"),
                "products": (bar.get("products") or [])[:10]},
        "shifts": [
            {"id": s.id, "employee": e.full_name if e else f"Администратор #{s.employee_id}",
             "started_at": _wall(s.started_at), "sales": _money(s.cash_sales) + _money(s.card_sales) + _money(s.mobile_sales),
             "cash": _money(s.cash_sales), "card": _money(s.card_sales), "online": _money(s.mobile_sales)}
            for s, e in open_shifts
        ],
        "warehouse": {"critical": len(critical), "items": [
            {"product": p.name, "club": c.name, "quantity": _money(b.quantity), "min_stock": _money(b.min_stock)}
            for b, p, c in critical
        ]},
        "control": {"open_without_report": sum(1 for _, _, r in open_reports if not r or r.status != "submitted"),
                     "violations": int(violations), "critical_stock": len(critical)},
    }
